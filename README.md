...
Note: parse at ingest is generally easier than creating derived columns. The collector examples should help but using predefined receivers for JournalD/SystemD, Ngnix, Apache etc tend to work better but these examples are showing how to parse and break down files that may not comply with what the receiver can provide. Beyond that using Honeycomb HTP, provides many sophistacted receivers with parsing setup before hand.
...
### Extract HTTP Access Log Fields using OTTL
...
### Work with Derived Columns/Calculated Fields and Conditions
  ...
  IF(
    ...
    IF(
      ...
      IF(
        ...
        REG_VALUE($body, '\\"msg\\":\\"([^\\"]+)\\"'),
        IF(
          REG_MATCH($body, 'fast query=\\"([^\\"]+)\\"'),
          REG_VALUE($body, 'fast query=\\"([^\\"]+)\\"'),
          IF(
